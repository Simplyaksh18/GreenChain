"""
Phase 7 — Drone observation tests.

Tests POST /drone/simulate and GET /drone/{farm_id}.
"""
import pytest


class TestDroneSimulate:
    ...
    def test_observation_fields_present(self, client, farmer_token, farm):
        ...
        assert "vegetation_cover_percent" in obs
        assert "standing_water_percent" in obs
        assert "anomaly_score" in obs
        assert "image_reference" in obs
    ...
    def test_farmer_cannot_simulate_other_farm(self, client, farmer_token, farm2):
        resp = client.post(
            "/drone/simulate",
            json={"farm_id": farm2.id, "number_of_observations": 1},
            headers={"Authorization": f"Bearer {farmer_token}"},
        )
...
class TestDroneList:
    def test_list_returns_observations(self, client, farmer_token, farm, drone_observations):
        resp = client.get(
            f"/drone/{farm.id}",
            headers={"Authorization": f"Bearer {farmer_token}"},
        )
    ...
    def test_list_sorted_by_date(self, client, farmer_token, farm, drone_observations):
        resp = client.get(
            f"/drone/{farm.id}",
            headers={"Authorization": f"Bearer {farmer_token}"},
        )
    ...
    def test_admin_can_list_any_farm(self, client, admin_token, farm, drone_observations):
        resp = client.get(
            f"/drone/{farm.id}",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
